...
use crate::routing::{AttemptSpec, Target, planner};
use crate::test::{spawn_sse_stub, test_provider_client, test_provider_runtime};
use crate::{ExecutionOptions, ResponseMode};
use crate::{MessageCreateInput, RuntimeErrorKind};

use super::stream_test_fixtures::response_from_events;

#[tokio::test]
async fn current_non_streaming_api_rejects_stream_requests() {
    let client = test_provider_client(ProviderKind::OpenAi);
