...
use crate::routing::{
    AttemptDisposition, AttemptExecutionOptions, AttemptPlanningError, AttemptSpec,
    PlanningRejectionKind, PlanningRejectionPolicy, Route, RoutePlanningFailureReason,
    RoutedPlanningResult, SkipReason, Target, plan_direct_attempt, plan_routed_attempt,
    plan_routed_execution, should_skip_planning_rejection,
};
use crate::test::{
    default_instance_id, test_provider_client, test_provider_client_with_streaming_support,
};
...
fn routed_planner_rejects_streaming_when_provider_capability_is_disabled() {
    ...
    let error = plan_routed_attempt(&client, &attempt, &test_task_request(), &execution)
...
fn skip_rejection_policy_advances_only_when_more_attempts_exist() {
    assert!(should_skip_planning_rejection(
...
fn routed_planning_failure_tracks_static_skip_history_and_reason() {
    ...
    match result {
        RoutedPlanningResult::PlanningFailure { failure, .. } => {
            assert_eq!(
                failure.reason,
...
fn routed_planning_fails_before_attempt_record_when_model_is_unresolved() {
    ...
    match result {
        RoutedPlanningResult::Fatal(error) => {
            assert_eq!(error.kind, RuntimeErrorKind::Configuration);
            assert!(error.message.contains("no model available"));
        }
        other => panic!("expected fatal planning error, got {other:?}"),
...
fn skipped_planning_records_never_carry_request_id_or_status_metadata() {
    ...
    let failure = match result {
        RoutedPlanningResult::PlanningFailure { failure, .. } => failure,
        other => panic!("expected route planning failure, got {other:?}"),
    };
